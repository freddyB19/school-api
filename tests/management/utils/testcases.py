...
class SocialMediaCreateTestCase(SocialMediaTestCase):
	def setUp(self):
		super().setUp()
...
class CoordinateTestCase(APITestCase):
	def setUp(self):
		self.client = APIClient()
...
class CoordianteCreateTestCase(CoordinateTestCase):
	def setUp(self):
		super().setUp()
		self.user_with_add_perm = create_user(role = 0)
		self.user_with_change_perm = create_user(role = 0)
		...
		self.user_with_add_perm.user_permissions.set(
			get_permissions(codenames = ["add_coordinate"])
		)

		self.user_with_change_perm.user_permissions.set(
			get_permissions(codenames = ["change_coordiante"])
		)
...
class RepositoryCreateTestCase(RepositoryTestCase):
	def setUp(self):
		super().setUp()
...
class RepositoryMediaFileTestCase(APITestCase):
	def setUp(self):
		self.client = APIClient()
		self.school = create_school()
		...
		self.user_with_view_perm.user_permissions.set(
			get_permissions(codenames = ['view_repositorymediafile'])
		)
		self.user_with_delete_perm.user_permissions.set(
			get_permissions(codenames = ['delete_repositorymediafile'])
		)
